...
def test_valid_organiser_returns_200_and_stats():
     conn = get_connection()
     seed(conn)
     conn.close()
     app = create_app()
     ...
     with TestClient(app) as client:
          ...
          assert isinstance(data["stats"]["organiser_rank"], int)
          assert isinstance(data["stats"]["best_attended_count"], int)


def test_invalid_organiser_id_returns_404():
    conn = get_connection()
    seed(conn)
    conn.close()
    app = create_app()
    ...
    with TestClient(app) as client:
        response = client.get("/api/organisers/9999/stats")
        assert response.status_code == 404
